...
def mlp(seq, window_size, epochs):
    ...
    model = nn.Sequential(nn.Linear(window_size, 32), nn.ReLU(), nn.Linear(32, 1))
...
def cnn(seq, window_size, epochs):
    ...
    test_input = (
        torch.tensor([seq[-window_size:]], dtype=torch.float32)
        .unsqueeze(0)
        .unsqueeze(0)
    )
    pred = model(test_input).item()
    ...
    try:
        ...
        for i in range(len(sys.argv)):
            match sys.argv[i].lower():
                ...
                case "--help" | "-h":
                    print(
                        """FastNN: A tool to quickly use prediction networks
    To input data:
